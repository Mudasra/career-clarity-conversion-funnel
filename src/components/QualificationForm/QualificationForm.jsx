...
export default function QualificationForm({ onSubmit: onSuccess }) {
  ...
  return (
    <div className="bg-white rounded-3xl p-8 shadow-sm border" style={{ borderColor: "#e0e5e9" }}>
        ...
        <div className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-semibold text-primary mb-2">
              What's your current job title / field? <span className="text-red-400">*</span>
            </label>
            <input
              {...register("currentJob", { required: "This field is required" })}
              type="text"
            ...
            />
            {errors.currentJob && (
              <p className="text-red-400 text-xs mt-1">{errors.currentJob.message}</p>
            )}
          </div>

          {/* Biggest challenge */}
          <QualificationChallengeField register={register} error={errors.biggestChallenge} />

          {/* Goal */}
          <div>
            <label className="block text-sm font-semibold text-primary mb-2">
              What does success look like for you in 90 days? <span className="text-red-400">*</span>
            </label>
            <textarea
              {...register("goal", { required: "Please share your goal", minLength: { value: 20, message: "Please be a bit more specific" } })}
              rows={3}
